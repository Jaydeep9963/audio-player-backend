...
import NotificationToken from './notificationToken/notificationToken.model';
...
class NotificationService {
  async sendToTokens(tokens: string[], title: string, body: string, data: NotificationData = {}): Promise<BatchResponse[]> {
    if (tokens.length === 0) {
  ...
  async sendToAllUsers(title: string, body: string, data: NotificationData = {}): Promise<BatchResponse[]> {
    try {
  ...
  async sendToSpecificToken(token: string, title: string, body: string, data: NotificationData = {}): Promise<string> {
    try {
      const message = {
        notification: { title, body },
        data,
  ...
  private async getAllUserTokens(): Promise<string[]> {
    try {
      ...
      const tokenDocuments = await NotificationToken.find().select('token').lean(); // Use lean() for better performance when only reading data
  ...
  async cleanupInvalidTokens(invalidTokens: string[]): Promise<void> {
    ...
    try {
      ...
      console.log(`Cleaned up ${deleteResult.deletedCount} invalid tokens from database`);
    } catch (error: unknown) {
  ...
  private extractInvalidTokens(results: BatchResponse[], originalTokens: string[]): string[] {
  ...
  async sendToAllUsersWithCleanup(title: string, body: string, data: NotificationData = {}): Promise<BatchResponse[]> {
    try {
      const userTokens = await this.getAllUserTokens();
      if (userTokens.length === 0) {
        throw new Error('No user tokens found');
